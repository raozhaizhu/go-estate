...
mock:
	...
	mockgen -destination=$(SESSION_SERVICE_DIR)/mock/cache.go -package=mock_service $(PROJECT_INTERNAL_PATH)/dao/cache SessionCache && \
	mockgen -destination=$(DAILY_DATA_SERVICE_DIR)/mock/service.go -package=mock_service $(PROJECT_INTERNAL_PATH)/dao/sqlc DailyDataStore && \
	mockgen -destination=$(USER_SERVICE_DIR)/mock/service.go -package=mock_service $(PROJECT_INTERNAL_PATH)/dao/sqlc UserStore && \
	mockgen -destination=$(SESSION_SERVICE_DIR)/mock/service.go -package=mock_service $(PROJECT_INTERNAL_PATH)/dao/sqlc AuthStore && \
